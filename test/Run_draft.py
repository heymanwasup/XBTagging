...
def main():
    ...
    if args.all or args.table:
        title_fmt = 'uncertainties of $\epsilon_{data}$, {0:}\% OP of MV2c10 tagger'        
        table_maker = BreakDown.BreakDown(output_path,args.version)
        for name,json_path in jsons.iteritems():
            title = TableTitle(name, title_fmt,['EffData','$\epsilon_{data}$','e_dt'])
            table_maker.GetTex(tex_name=name, input_json=json_path, intervals=intervals, title=title, label='uncertainties')
