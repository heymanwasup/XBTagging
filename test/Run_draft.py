...
parser.add_argument('-p','--parallel', action='store_true', help='work parallelly')
...
def main():
    ...
    intervals = toolkit.json_load('./data/Overall_Info.json')['binnings'][toolkit.json_load(args.config_file)['binnings']][1:]

    if args.all or args.table:
        MakeTable(output_path,intervals,jsons,args)
        
    if args.all or args.cdi_inputs:
        MakeCDIInputs(output_path,jsons,jsons_raw,args)

def CollectJsonFiles(output_path):
    ...
    return json_path,jsons,jsons_raw

def MakeTable(output_path,intervals,jsons,args):
...
def MakeCDIInputs(output_path,jsons,jsons_raw,args):
